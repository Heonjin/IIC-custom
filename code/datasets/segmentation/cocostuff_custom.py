...
__all__ = ["Coco10kFull", "Coco10kFew", "Coco164kFull", "Coco164kFew",
           "Coco164kCuratedFew", "Coco164kCuratedFull", "Custom"]
...
class _Custom():
  ...
  def _prepare_test(self, index, img, label):
    ...
    # convert to coarse if required, reindex to [0, gt_k -1], and get mask
    label, mask = self._filter_label(label)   # label.max() == 254 -> label.max() == gt_k -1
...
class _Custom2(_Custom):
  def __init__(self, **kwargs):
    super(_Custom2, self).__init__(**kwargs)
...
#    self._fine_to_few_dict = self._make_fine_to_few_dict()
  ...
  def _filter_label(self, label):
    ...
    mini = label.min()
    maxi = label.max()
    threshold = (maxi-mini)//(self.gt_k+1)

    for i in range(self.gt_k+1):
      new_label_map[i * threshold <= label] = i-1
...
class _Custom3(_Custom):
  ...
  def __init__(self, **kwargs):
    super(_Custom3, self).__init__(**kwargs)
...
class Custom(_Custom2,_Custom3):
  def __init__(self, **kwargs):
